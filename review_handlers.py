...
from keyboards import create_review_menu, create_back_to_main_menu
...
class ReviewHandlers:
    ...
    async def product_reviews_handler(self, callback: CallbackQuery, state: FSMContext):
        """Обработчик просмотра отзывов о товаре"""
        try:
            product_id = int(callback.data.split("_")[2])
            product = await self.db.get_product(product_id)
            reviews = await self.db.get_product_reviews(product_id, limit=10)
    ...
    async def review_order_handler(self, callback: CallbackQuery, state: FSMContext):
        """Обработчик начала отзыва"""
        try:
            order_id = int(callback.data.split("_")[2])
            
            # Проверяем, может ли пользователь оставить отзыв
            if not await self.db.can_review_order(callback.from_user.id, order_id):
    ...
    async def rate_handler(self, callback: CallbackQuery, state: FSMContext):
    ...
    async def process_review_comment(self, message: Message, state: FSMContext):
        ...
        try:
            ...
            # Добавляем отзыв
            await self.db.add_review(message.from_user.id, product_id, order_id, rating, comment)
